...
struct JsonVal {
    enum Kind { Null, BoolVal, Number, String, Object, Array };
    Kind kind{Null};
    bool b{false};
    double num{0};
    std::string str;
    ...
    bool is(const char *k) const { return find(k) != nullptr; }
...
private:
    ...
    static char peek() { return *ctx_; }
    static char next() { return *ctx_++; }
    static void skip_ws() {
    ...
    static std::optional<JsonVal> parse_object() {
        next(); // '{'
        JsonVal v; v.kind = JsonVal::Object;
        skip_ws();
    ...
    static std::optional<JsonVal> parse_array() {
        next(); // '['
        JsonVal v; v.kind = JsonVal::Array;
        skip_ws();
        if (peek() == ']') { next(); return v; }
        for (;;) {
    ...
    static std::optional<JsonVal> parse_string() {
        next(); // '"'
        JsonVal v; v.kind = JsonVal::String;
        while (peek() != '"') {
            if (peek() == '\\') { next(); }
            if (peek() == 0) return std::nullopt;
            v.str += next();
        }
        next(); // '"'
        return v;
    ...
    static std::optional<JsonVal> parse_number() {
        JsonVal v; v.kind = JsonVal::Number;
        char *end = nullptr;
        v.num = std::strtod(ctx_, &end);
        if (end == ctx_) return std::nullopt;
        ctx_ = end;
        return v;
    }

    static std::optional<JsonVal> parse_bool() {
        JsonVal v; v.kind = JsonVal::BoolVal;
        if (std::strncmp(ctx_, "true", 4) == 0) { v.b = true;  ctx_ += 4; return v; }
        if (std::strncmp(ctx_, "false", 5) == 0) { v.b = false; ctx_ += 5; return v; }
        return std::nullopt;
    }

    static std::optional<JsonVal> parse_null() {
        if (std::strncmp(ctx_, "null", 4) != 0) return std::nullopt;
        ctx_ += 4;
        return JsonVal{};
    }
};
...
d20::DcTransferLimits parse_dc_limits(const JsonVal &v) {
    ...
    if (c) {
        auto *p = c->find("power");
        if (p) { lim.charge_limits.power.max = {static_cast<int16_t>(p->get_num("max")), 0};
                 lim.charge_limits.power.min = {static_cast<int16_t>(p->get_num("min")), 0}; }
        auto *cur = c->find("current");
        if (cur) { lim.charge_limits.current.max = {static_cast<int16_t>(cur->get_num("max")), 0}; }
    }
    auto *volt = v.find("voltage");
    if (volt) { lim.voltage.max = {static_cast<int16_t>(volt->get_num("max")), 0}; }
    auto *disc = v.find("discharge_limits");
    if (disc) {
        auto &dl = lim.discharge_limits.emplace();
        auto *dp = disc->find("power");
        if (dp) { dl.power.max = {static_cast<int16_t>(dp->get_num("max")), 0};
                  dl.power.min = {static_cast<int16_t>(dp->get_num("min")), 0}; }
        auto *dc = disc->find("current");
        if (dc) { dl.current.max = {static_cast<int16_t>(dc->get_num("max")), 0}; }
    }
    return lim;
}
...
d20::AcTransferLimits parse_ac_limits(const JsonVal &v) {
    ...
    if (cp) { lim.charge_power.max = {static_cast<int16_t>(cp->get_num("max")), 0};
...
d20::EvseSetupConfig parse_evse_config(const std::string &json) {
    ...
    // Energy services
    for (auto &s : root->get_str("energy_services")) (void)s;
    if (auto *es = root->find("energy_services")) {
        for (auto &v : es->items) {
            auto s = v.str;
            if (s == "AC")       cfg.supported_energy_services.push_back(dt::ServiceCategory::AC);
            else if (s == "DC")  cfg.supported_energy_services.push_back(dt::ServiceCategory::DC);
            else if (s == "AC_BPT") cfg.supported_energy_services.push_back(dt::ServiceCategory::AC_BPT);
            else if (s == "DC_BPT") cfg.supported_energy_services.push_back(dt::ServiceCategory::DC_BPT);
            else if (s == "AC_DER") cfg.supported_energy_services.push_back(dt::ServiceCategory::AC_DER);
            else if (s == "MCS")    cfg.supported_energy_services.push_back(dt::ServiceCategory::MCS);
            else if (s == "MCS_BPT") cfg.supported_energy_services.push_back(dt::ServiceCategory::MCS_BPT);
        }
    }
    ...
    if (auto *as = root->find("auth_services")) {
        for (auto &v : as->items)
            cfg.authorization_services.push_back(
                v.str == "PnC" ? dt::Authorization::PnC : dt::Authorization::EIM);
    }

    // Control / mobility modes
    if (auto *modes = root->find("control_mobility_modes")) {
        for (auto &m : modes->items) {
            auto cm = m.get_str("control_mode") == "Dynamic" ? dt::ControlMode::Dynamic
                                                              : dt::ControlMode::Scheduled;
            auto mm = m.get_str("mobility_mode") == "ProvidedBySecc"
                          ? dt::MobilityNeedsMode::ProvidedBySecc
                          : dt::MobilityNeedsMode::ProvidedByEvcc;
            cfg.control_mobility_modes.push_back({cm, mm});
        }
    } else {
        auto cm = root->get_str("control_mode") == "Dynamic" ? dt::ControlMode::Dynamic
                                                              : dt::ControlMode::Scheduled;
        auto mm = root->get_str("mobility_mode") == "ProvidedBySecc"
                      ? dt::MobilityNeedsMode::ProvidedBySecc
                      : dt::MobilityNeedsMode::ProvidedByEvcc;
        cfg.control_mobility_modes.push_back({cm, mm});
    }
...
d20::ControlEvent parse_control_event(const std::string &json) {
    ...
    if (kind == "UpdateDynamicModeParameters") {
        ...
        if (dt > 0) p.departure_time = static_cast<uint32_t>(dt);
    ...
    if (kind == "AcTargetPower") {
        ...
        if (v != 0) p.target_active_power = dt::RationalNumber{
            static_cast<int16_t>(v), 0};
        v = root->get_num("target_reactive_power");
        if (v != 0) p.target_reactive_power = dt::RationalNumber{
            static_cast<int16_t>(v), 0};
        return p;
    }
    ...
    if (kind == "AcPresentPower") {
        ...
        if (v != 0) p.present_active_power = dt::RationalNumber{
            static_cast<int16_t>(v), 0};
        return p;
    }
    ...
    if (kind == "EnergyServices") {
        ...
        if (auto *arr = root->find("services")) {
            for (auto &s : arr->items) {
                if (s.str == "AC") svcs.push_back(dt::ServiceCategory::AC);
                else if (s.str == "DC") svcs.push_back(dt::ServiceCategory::DC);
                else if (s.str == "DC_BPT") svcs.push_back(dt::ServiceCategory::DC_BPT);
            }
        }
...
struct JsonBuf {
    ...
    void start_obj() { buf += '{'; }
    void end_obj()   { if (buf.back() == ',') buf.pop_back(); buf += '}'; }
    void key(const char *k) { buf += '"'; buf += k; buf += "\":"; }
    void str_val(const char *v) { buf += '"'; buf += v; buf += '"'; buf += ','; }
    void str_val(const std::string &v) { str_val(v.c_str()); }
    void int_val(int v) { buf += std::to_string(v); buf += ','; }
    void bool_val(bool v) { buf += v ? "true," : "false,"; }
};

void emit_signal(JsonBuf &b, const std::string &session_id, const char *sig) {
    b.start_obj();
    b.key("type"); b.str_val("signal");
    b.key("session_id"); b.str_val(session_id);
    b.key("signal"); b.str_val(sig);
    b.end_obj();
}
...
iso15118::session::feedback::Callbacks make_callbacks(iso15118_session_t &s) {
    ...
    cb.signal = [&s](iso15118::session::feedback::Signal sig) {
        ...
        switch (sig) {
        case iso15118::session::feedback::Signal::CHARGE_LOOP_STARTED:  name = "CHARGE_LOOP_STARTED"; break;
        case iso15118::session::feedback::Signal::CHARGE_LOOP_FINISHED: name = "CHARGE_LOOP_FINISHED"; break;
        case iso15118::session::feedback::Signal::DLINK_TERMINATE:      name = "DLINK_TERMINATE"; break;
        case iso15118::session::feedback::Signal::DLINK_PAUSE:          name = "DLINK_PAUSE"; break;
        case iso15118::session::feedback::Signal::DLINK_ERROR:          name = "DLINK_ERROR"; break;
        case iso15118::session::feedback::Signal::DC_OPEN_CONTACTOR:    name = "DC_OPEN_CONTACTOR"; break;
        case iso15118::session::feedback::Signal::AC_CLOSE_CONTACTOR:   name = "AC_CLOSE_CONTACTOR"; break;
        case iso15118::session::feedback::Signal::AC_OPEN_CONTACTOR:    name = "AC_OPEN_CONTACTOR"; break;
        case iso15118::session::feedback::Signal::REQUIRE_AUTH_EIM:     name = "REQUIRE_AUTH_EIM"; break;
        case iso15118::session::feedback::Signal::START_CABLE_CHECK:    name = "START_CABLE_CHECK"; break;
        case iso15118::session::feedback::Signal::SETUP_FINISHED:       name = "SETUP_FINISHED"; break;
        case iso15118::session::feedback::Signal::PRE_CHARGE_STARTED:   name = "PRE_CHARGE_STARTED"; break;
        }
    ...
    cb.v2g_message = [&s](const m20::Type &t) {
        JsonBuf b;
        b.start_obj();
        b.key("type"); b.str_val("v2g_message");
        b.key("session_id"); b.str_val(s.session_id_str);
        b.key("msg_type"); b.int_val(static_cast<int>(t));
        b.end_obj();
        s.emit(b.buf);
    };
    ...
    cb.evccid = [&s](const std::string &id) {
        JsonBuf b;
        b.start_obj();
        b.key("type"); b.str_val("evcc_id");
        b.key("session_id"); b.str_val(s.session_id_str);
        b.key("evcc_id"); b.str_val(id);
        b.end_obj();
        s.emit(b.buf);
    };
    ...
    cb.selected_protocol = [&s](const std::string &proto) {
        JsonBuf b;
        b.start_obj();
        b.key("type"); b.str_val("selected_protocol");
        b.key("session_id"); b.str_val(s.session_id_str);
        b.key("protocol"); b.str_val(proto);
        b.end_obj();
        s.emit(b.buf);
    };
    ...
    cb.selected_service_parameters = [&s](const d20::SelectedServiceParameters &sp) {
        JsonBuf b;
        b.start_obj();
        b.key("type"); b.str_val("selected_service");
        b.key("session_id"); b.str_val(s.session_id_str);
        b.key("energy_service"); b.int_val(static_cast<int>(sp.selected_energy_service));
        b.key("control_mode"); b.int_val(static_cast<int>(sp.selected_control_mode));
        b.key("mobility_mode"); b.int_val(static_cast<int>(sp.selected_mobility_needs_mode));
        b.key("pricing"); b.int_val(static_cast<int>(sp.selected_pricing));
        b.end_obj();
        s.emit(b.buf);
    };
    ...
    cb.ev_termination = [&s](const std::string &code, const std::string &msg) {
        JsonBuf b;
        b.start_obj();
        b.key("type"); b.str_val("error");
        b.key("session_id"); b.str_val(s.session_id_str);
        b.key("code"); b.str_val(code);
        b.key("message"); b.str_val(msg);
        b.end_obj();
        s.emit(b.buf);
    };
...
void iso15118_session_set_callback(iso15118_session_t *s,
                                    iso15118_event_fn fn, void *userdata) {
    s->event_fn = fn;
...
int iso15118_session_poll(iso15118_session_t *s) {
    ...
    try {
        ...
        if (!s->session) {
            ...
            auto conn = std::make_unique<io::ConnectionPlain>(
                s->poll_manager, "eth0"); // TODO: configurable interface

            auto callbacks = make_callbacks(*s);
            std::optional<d20::PauseContext> pause_ctx{std::nullopt};

            s->session = std::make_unique<iso15118::Session>(
                std::move(conn), std::move(session_config), callbacks, pause_ctx);
        }
...
void iso15118_session_push_event(iso15118_session_t *s, const char *event_json) {
    ...
    if (!s->session) {
        std::snprintf(g_last_error, sizeof(g_last_error),
                      "push_event: session not yet created (call poll first)");
        return;
    }
    try {
        ...
        std::snprintf(g_last_error, sizeof(g_last_error),
                      "push_event parse error: %s", e.what());
    }
}
