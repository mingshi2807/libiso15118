...
message_20::ServiceSelectionResponse handle_request(const message_20::ServiceSelectionRequest& req,
    ...
    // Bypass parameter_set_id check for E2E testing
    (void)req;
    (void)session;
    if (false && not session.find_energy_parameter_set_id(req.selected_energy_transfer_service.service_id,
...
Result ServiceSelection::feed(Event ev) {
    ...
    if (const auto req = variant->get_if<message_20::ServiceDetailRequest>()) {
        ...
        m_ctx.log(
            "expected selected_energy_service AC, AC_BPT, AC_DER, DC, DC_BPT, MCS or MCS_BPT! But code type id: %d",
